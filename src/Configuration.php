...
class Configuration implements ConfigurationInterface
{
    ...
    public function getConfigTreeBuilder()
    {
        ...
        $defaultOrderedRemediations = Constants::ORDERED_REMEDIATIONS;
        ...
        $rootNode
            ->children()
                ...
                ->end()
                ->arrayNode('ordered_remediations')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->enumNode(0)->isRequired()
                            ->values($defaultOrderedRemediations)
                            ->defaultValue($defaultOrderedRemediations[0])
                        ->end()
                        ->enumNode(1)->isRequired()
                            ->values($defaultOrderedRemediations)
                            ->defaultValue($defaultOrderedRemediations[1])
                        ->end()
                        ->enumNode(2)->isRequired()
                            ->values($defaultOrderedRemediations)
                            ->defaultValue($defaultOrderedRemediations[2])
                        ->end()
                    ->end()
                ->end()
